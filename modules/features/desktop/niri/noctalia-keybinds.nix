{
  flake.modules.homeManager.noctalia-keybinds =
    { config, ... }:
    let
      noctalia = cmd: args: {
        action.spawn = [
          "noctalia"
          "msg"
          cmd
        ]
        ++ args;
      };
      noctalia' =
        cmd: args: extraAttrs:
        (noctalia cmd args) // extraAttrs;
    in
    {
      programs.niri.settings.binds = {
        "Mod+Space" = noctalia "panel-toggle" [ "launcher" ];
        "Mod+V" = noctalia "panel-toggle" [ "clipboard" ];
        "Mod+Return".action.spawn = [ "wezterm" ];
        "Mod+Shift+Return".action.spawn = [ "firefox" ];
        "Mod+W".action.spawn = [ "thunar" ];

        "Mod+Shift+Backslash".action.show-hotkey-overlay = [ ];
        "Mod+N" = noctalia "panel-toggle" [ "control-center" "notifications" ];
        "Mod+P" = noctalia "panel-toggle" [ "control-center" ];
        "Mod+X" = noctalia "panel-toggle" [ "session" ];
        "Mod+Comma" = noctalia "settings-toggle" [ ];
        "Ctrl+Alt+Delete" = noctalia "window-switcher" [ ];

        "Mod+Q".action.close-window = [ ];
        "Mod+F".action.fullscreen-window = [ ];
        "Mod+Shift+F".action.maximize-window-to-edges = [ ];
        "Mod+Shift+V".action.toggle-window-floating = [ ];
        "Mod+Shift+Print".action.expand-column-to-available-width = [ ];
        "Mod+Period".action.switch-preset-column-width = [ ];
        "Mod+Shift+E".action.quit = [ ];
        "Print" = noctalia "screenshot-region" [ ];
        "Mod+Print" = noctalia "screenshot-fullscreen" [ ];
        "Mod+Y" = noctalia "panel-toggle" [ "wallpaper" ];

        "Mod+Alt+L" = noctalia' "session" [ "lock" ] {
          allow-when-locked = true;
        };
        "Mod+Shift+Q" = with config.lib.niri.actions; {
          action = spawn "noctalia" "msg" "session" "logout";
          allow-when-locked = true;
        };

        "Mod+O".action.toggle-overview = [ ];

        "Mod+1".action.focus-workspace = 1;
        "Mod+2".action.focus-workspace = 2;
        "Mod+3".action.focus-workspace = 3;
        "Mod+4".action.focus-workspace = 4;
        "Mod+5".action.focus-workspace = 5;
        "Mod+6".action.focus-workspace = 6;

        "Mod+Shift+1".action.move-column-to-workspace = 1;
        "Mod+Shift+2".action.move-column-to-workspace = 2;
        "Mod+Shift+3".action.move-column-to-workspace = 3;
        "Mod+Shift+4".action.move-column-to-workspace = 4;
        "Mod+Shift+5".action.move-column-to-workspace = 5;
        "Mod+Shift+6".action.move-column-to-workspace = 6;

        "Mod+MouseMiddle".action.maximize-column = [ ];
        "Mod+Shift+C".action.center-window = [ ];
        "Mod+Shift+Comma".action.set-column-width = "-10%";
        "Mod+Shift+Period".action.set-column-width = "+10%";
        "Mod+Shift+Semicolon".action.set-window-height = "+10%";
        "Mod+Shift+Slash".action.set-window-height = "-10%";
        "Mod+J".action.consume-or-expel-window-left = [ ];
        "Mod+Semicolon".action.consume-or-expel-window-right = [ ];

        "Mod+Shift+Left".action.focus-column-left = [ ];
        "Mod+Shift+Right".action.focus-column-right = [ ];
        "Mod+Shift+Down".action.focus-window-or-workspace-down = [ ];
        "Mod+Shift+Up".action.focus-window-or-workspace-up = [ ];

        "Mod+WheelScrollLeft".action.focus-workspace-up = [ ];
        "Mod+WheelScrollRight".action.focus-workspace-down = [ ];

        "Mod+WheelScrollDown".action.focus-column-right = [ ];
        "Mod+WheelScrollUp".action.focus-column-left = [ ];
        "Mod+Shift+WheelScrollDown".action.swap-window-right = [ ];
        "Mod+Shift+WheelScrollUp".action.swap-window-left = [ ];
        "Mod+Ctrl+F".action.move-column-to-first = [ ];
        "Mod+Ctrl+L".action.move-column-to-last = [ ];

        "XF86AudioRaiseVolume" = noctalia' "volume-up" [ "3" ] { allow-when-locked = true; };
        "XF86AudioLowerVolume" = noctalia' "volume-down" [ "3" ] { allow-when-locked = true; };
        "XF86AudioMute" = noctalia' "volume-mute" [ ] { allow-when-locked = true; };
        "XF86AudioMicMute" = noctalia' "mic-mute" [ ] { allow-when-locked = true; };
        "XF86AudioNext" = noctalia' "media" [ "next" ] { allow-when-locked = true; };
        "XF86AudioPause" = noctalia' "media" [ "toggle" ] {
          allow-when-locked = true;
        };
        "XF86AudioPlay" = noctalia' "media" [ "toggle" ] {
          allow-when-locked = true;
        };
        "XF86AudioPrev" = noctalia' "media" [ "previous" ] {
          allow-when-locked = true;
        };
        "XF86MonBrightnessUp" = noctalia' "brightness-up" [ "5" ] {
          allow-when-locked = true;
        };
        "XF86MonBrightnessDown" = noctalia' "brightness-down" [ "5" ] {
          allow-when-locked = true;
        };
      };
    };

}
